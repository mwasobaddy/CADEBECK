...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public $isLoadingStatus = false; // Added for status updates
    ...
    public function updateStatus($applicationId, $newStatus): void
    {
        ...
        if ($application->isInCooldown()) {
            $remainingSeconds = $application->getCooldownRemainingSeconds();
            $this->dispatch('notify', [
                'type' => 'warning', 
                'message' => __('Please wait :seconds more seconds before changing status again.', ['seconds' => $remainingSeconds])
            ]);
        ...
        try {
            // Send notification directly to the applicant's email
            Notification::route('mail', $application->email)
                       ->notify(new ApplicationStatusNotification($application, $oldStatus, $newStatus));
            
            $this->dispatch('notify', [
                'type' => 'success', 
                'message' => __('Application status updated to :status and email sent successfully.', ['status' => $newStatus])
            ]);
        } catch (\Exception $e) {
            ...
            $this->dispatch('notify', [
                'type' => 'success', 
                'message' => __('Application status updated to :status successfully. (Email notification failed)', ['status' => $newStatus])
            ]);
        }
    ...
    <div class="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-full shadow-lg p-4 mb-8 z-10 relative border border-blue-100 dark:border-zinc-800 ring-1 ring-blue-200/30 dark:ring-zinc-700/40">
        <nav class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('job.applications', $jobAdvertId) }}" class="border rounded-full py-2 px-4 hover:bg-zinc-100 dark:hover:bg-zinc-800 {{ request()->routeIs('job.applications') ? 'bg-green-600 dark:bg-green-700 text-white dark:text-zinc-200 border-green-400 dark:border-green-500' : '' }}" wire:navigate>
                    {{ __('Job Applications') }}
                </a>
            </div>
        </nav>
            ...
            @if ($showFilters ?? false)
                <div class="flex flex-wrap gap-6 mt-6 items-center animate-fade-in">
                    <flux:select wire:model.live="filterStatus" class="!ps-3 pe-4 !py-2 !rounded-full border !border-blue-200 dark:!border-indigo-700 !focus:ring-2 !focus:ring-blue-400 dark:!bg-zinc-800/80 dark:!text-white !shadow-sm !bg-white/80 dark:!bg-zinc-900/80 !backdrop-blur-md !w-fit !outline-none">
                        ...
                        <flux:select.option value="Shortlisted">{{ __('Shortlisted') }}</flux:select.option>
                        <flux:select.option value="Rejected">{{ __('Rejected') }}</flux:select.option>
                        <flux:select.option value="Invited">{{ __('Invited') }}</flux:select.option>
                    </flux:select>
                    ...
                    @if($this->shouldShowSkeleton())
                        @for($i = 0; $i < $perPage; $i++)
                            <tr class="animate-pulse border-b border-gray-200 dark:border-gray-700">
                                ...
                                <td class="px-5 py-4"><div class="h-6 w-28 bg-blue-100 dark:bg-zinc-800 rounded"></div></td>
                                <td class="px-5 py-4"><div class="h-4 w-24 bg-blue-100 dark:bg-zinc-800 rounded"></div></td>
                                <td class="px-5 py-4"><div class="flex gap-2"><div class="h-8 w-8 bg-gray-100 dark:bg-zinc-800 rounded"></div><div class="h-8 w-8 bg-gray-100 dark:bg-zinc-800 rounded"></div><div class="h-8 w-8 bg-gray-100 dark:bg-zinc-800 rounded"></div></div></td>
                            </tr>
                        @endfor
                    @else
                        @forelse ($this->applications ?? [] as $application)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-zinc-800/50 transition-colors" wire:loading.class.delay="opacity-50 dark:opacity-40">
                                <td class="px-5 py-4"><button type="button"
                                        wire:click="toggleSelection({{ $application->id }})"
                                        class="rounded focus:ring-2 focus:ring-pink-400 transition-colors duration-200
                                            @if(in_array($application->id, $selected))
                                                bg-pink-500 text-white p-[2px]
                                            @else
                                                bg-transparent text-pink-500 border border-gray-500 p-[6px]
                                            @endif
                                ...
                                <td class="px-5 py-4">
                                    @can('vet_candidate')
                                        <div class="relative">
                                            @php
                                                $isInCooldown = $application->isInCooldown();
                                                $remainingSeconds = $isInCooldown ? $application->getCooldownRemainingSeconds() : 0;
                                            @endphp
                                            
                                            <select 
                                                wire:change="updateStatus({{ $application->id }}, $event.target.value)"
                                                {{ $isInCooldown ? 'disabled' : '' }}
                                                class="w-full px-3 py-2 text-sm rounded-lg border transition-colors
                                                    {{ $isInCooldown 
                                                        ? 'border-orange-300 bg-orange-50 text-orange-900 dark:border-orange-600 dark:bg-orange-900/20 dark:text-orange-200 cursor-not-allowed' 
                                            ...
                                            >
                                                ...
                                                <option value="Invited" {{ $application->status === 'Invited' ? 'selected' : '' }}>
                                                    {{ __('Invited') }}
                                                </option>
                                            </select>
                                            
                                            @if($isInCooldown)
                                                <div class="absolute -bottom-6 left-0 right-0 text-xs text-orange-600 dark:text-orange-400 text-center">
                                                    <div class="flex items-center justify-center gap-1">
                                                        <svg class="w-3 h-3 animate-spin" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                            <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" fill="none" stroke-dasharray="31.416" stroke-dashoffset="31.416" class="animate-pulse"></circle>
                                                        </svg>
                                                        <span data-cooldown-seconds="{{ $remainingSeconds }}">{{ $remainingSeconds }}s</span>
                                                    </div>
                                                </div>
                                            @endif
                                        </div>
