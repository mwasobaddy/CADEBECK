...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public function save(): void
    {
        ...
        session()->flash('status', $this->editing ? 'Designation updated.' : 'Designation created.');
        $this->redirectRoute('organisation.designation_manager');
    }
};
?>
...
<div class="relative max-w-6xl mx-auto md:px-4 md:py-8">
    <div class="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-full shadow-lg p-4 mb-8">
        <nav class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                <a href="#" class="border rounded-full py-2 px-4">Designation List</a>
                <a href="#" class="border rounded-full py-2 px-4 bg-green-600 text-white">Create Designation</a>
            </div>
        </nav>
    </div>
    ...
    <div class="relative z-10 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-xl shadow-2xl p-8">
        <div class="flex items-center gap-3 mb-8">
            <h1 class="text-3xl font-extrabold">{{ $editing ? 'Edit Designation' : 'Create Designation' }}</h1>
        </div>
        <form wire:submit.prevent="save" class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
                ...
                <input type="text" wire:model="form.title" class="w-full px-4 py-3 rounded-2xl border" required />
            </div>
            <div>
                <label class="block mb-2 text-sm font-semibold">Level</label>
                <input type="text" wire:model="form.level" class="w-full px-4 py-3 rounded-2xl border" />
            </div>
