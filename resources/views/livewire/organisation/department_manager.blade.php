...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        $query = Department::query()->with(['branch', 'branch.location']);
    ...
    public function updateSelectAllState(): void
    {
        ...
        if ($departments && $departments->count() > 0) {
            $currentPageIds = $departments->pluck('id')->toArray();
    ...
    public function exportSelected(): void
    {
        ...
        $departments = Department::whereIn('id', $this->selected)->with(['branch', 'branch.location'])->get();
        $csvData = "ID,Name,Code,Branch,Location,Created At\n";
        foreach ($departments as $department) {
            $csvData .= '"' . $department->id . '","' .
                str_replace('"', '""', $department->name) . '","' .
                str_replace('"', '""', $department->code) . '","' .
                str_replace('"', '""', $department->branch?->name) . '","' .
                str_replace('"', '""', $department->branch?->location?->name) . '","' .
        ...
        $this->dispatch('notify', ['type' => 'success', 'message' => __('Selected departments exported successfully.')]);
    }
    ...
    public function exportAll(): void
    {
        ...
        $this->dispatch('download-csv', [
            'data' => $csvData,
            'filename' => 'all_departments_' . now()->format('Y-m-d_H-i-s') . '.csv'
        ]);
    ...
    public function editConfirmed(): void
    {
        ...
        $department = Department::findOrFail($this->pendingEditId);
        $this->showEditModal = false;
        $this->isLoadingEdit = false;
        $this->redirectRoute('department.edit', ['id' => $department->id]);
    }

    public function deleteConfirmed(): void
    {
        $this->isLoadingDelete = true;
        Department::findOrFail($this->pendingDeleteId)->delete();
        $this->resetForm();
        $this->showDeleteModal = false;
        $this->isLoadingDelete = false;

        $this->dispatch('notify', ['type' => 'success', 'message' => __('Department deleted successfully.')]);
    }
    ...
    public function createNewDepartment(): void
    {
        $this->redirectRoute('department.create');
    }
    ...
    <div class="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-full shadow-lg p-4 mb-8 z-10 relative border border-blue-100 dark:border-zinc-800 ring-1 ring-blue-200/30 dark:ring-zinc-700/40">
        <nav class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                <a href="{{ route('department.manage') }}" class="border rounded-full py-2 px-4 hover:bg-zinc-100 dark:hover:bg-zinc-800 {{ request()->routeIs('department.manage') ? 'bg-green-600 dark:bg-green-700 text-white dark:text-zinc-200 border-green-400 dark:border-green-500' : '' }}" wire:navigate>
                    {{ __('Department List') }}
                </a>
                <a href="{{ route('department.create') }}" class="border rounded-full py-2 px-4 hover:bg-zinc-100 dark:hover:bg-zinc-800 {{ request()->routeIs('department.create') ? 'bg-green-600 dark:bg-green-700 text-white dark:text-zinc-200 border-green-400 dark:border-green-500' : '' }}" wire:navigate>
                    {{ __('Add Department') }}
                </a>
            </div>
        </nav>
    ...
    @can('manage_department')
        ...
        <div class="relative bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-xl shadow-2xl p-6 transition-all duration-300 hover:shadow-3xl border border-blue-100 dark:border-zinc-800 ring-1 ring-blue-200/30 dark:ring-zinc-700/40">
            <div class="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @can('create_department')
                        <button type="button" wire:click="createNewDepartment"
                            ...
                            <span class="hidden lg:inline">{{ __('Add Department') }}</span>
                        </button>
                    @endcan
                </div>
            </div>
            ...
            @if (count($selected) > 0)
                <div class="flex items-center justify-between flex-wrap mt-6 p-4 bg-gradient-to-r from-blue-50/80 to-indigo-50/80 dark:from-zinc-800/50 dark:to-zinc-700/50 rounded-xl border border-blue-200 dark:border-zinc-700 backdrop-blur-sm">
                    <div class="flex items-center gap-2 py-2">
                        ...
                        @if(count($selected) < ($this->departments ? $this->departments->total() : 0))
                            <button type="button" wire:click="selectAllData"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline">
                                {{ __('Select all') }} {{ $this->departments ? $this->departments->total() : 0 }} {{ __('items') }}
                            </button>
                        @endif
                    </div>
                    <div class="flex items-center gap-3">
            ...
            <div class="overflow-x-auto bg-transparent mt-6">
                        ...
                        @else
                            @forelse (($this->departments ?? []) as $department)
                                <tr class="hover:bg-gray-100 dark:hover:bg-white/20 group border-b border-gray-200 dark:border-gray-700 transition-all duration-500 ease-in-out" wire:loading.class.delay="opacity-50 dark:opacity-40">
                                    <td class="px-5 py-4">
                                        <input type="checkbox" wire:model.live="selected" value="{{ $department->id }}" class="accent-pink-500 rounded focus:ring-2 focus:ring-pink-400" />
                                    </td>
                                    <td class="px-5 py-4 text-gray-900 dark:text-white font-bold max-w-xs truncate flex items-center gap-3">
                                        <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-green-100 dark:bg-zinc-800 text-green-600 dark:text-green-300 font-bold text-lg">
                                            {{ strtoupper(Str::substr($department->name, 0, 1) . (Str::substr($department->name, 1, 1) ?: Str::substr($department->code ?? 'D', 0, 1))) }}
                                        </span>
                                        <span>
                                            <span class="block font-semibold text-base">{{ $department->name }}</span>
                                            <span class="block text-xs text-gray-500 dark:text-gray-400">{{ $department->code }}</span>
                                        </span>
                                    </td>
                                    <td class="px-5 py-4 font-semibold">{{ $department->code }}</td>
                                    <td class="px-5 py-4">{{ $department->branch?->name ?? '-' }}</td>
                                    <td class="px-5 py-4">{{ $department->branch?->location?->name ?? '-' }}</td>
                                    <td class="px-5 py-4 font-semibold">
                                        <span class="truncate text-blue-600 dark:text-blue-400">
                                            {{ $department->created_at ? \Carbon\Carbon::parse($department->created_at)->translatedFormat('j M Y') : '' }}
                                        </span>
                                    </td>
                                    <td class="px-5 py-4">
                                        <span class="flex gap-2">
                                            @can('edit_department')
                                                <flux:button
                                                    wire:click="confirmEdit({{ $department->id }})"
                                                    variant="primary"
                                                    color="blue"
                                                    size="sm"
                                                    icon="pencil-square"
                ...
                <div class="mt-6">
                    @if($this->departments && !$this->shouldShowSkeleton())
        ...
        @if ($showEditModal)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition">
                <div class="bg-white dark:bg-zinc-900 backdrop-blur-xl rounded-2xl shadow-2xl p-8 max-w-md w-full border border-gray-100 dark:border-zinc-800">
                    ...
                    <p class="mb-6 text-zinc-700 dark:text-zinc-300">
                        {{ __('Are you sure you want to edit this department?') }}</p>
                    <div class="flex justify-end gap-3">
            ...
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition">
                <div class="bg-gradient-to-br from-pink-50/80 via-white/80 to-red-100/80 dark:from-zinc-900/80 dark:via-zinc-800/80 dark:to-zinc-900/80 backdrop-blur-xl rounded-xl shadow-2xl p-10 max-w-md w-full border border-pink-200 dark:border-zinc-800">
                    ...
                    <p class="mb-6 text-zinc-700 dark:text-zinc-300 font-semibold">
                        {{ __('Are you sure you want to delete the selected departments? This action cannot be undone.') }}
                    </p>
                    <div class="flex justify-end gap-3">
