...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public function editConfirmed(): void
    {
        ...
        $designation = Designation::findOrFail($this->pendingEditId);
        $this->showEditModal = false;
        $this->isLoadingEdit = false;
        $this->redirectRoute('designation.edit', ['id' => $designation->id]);
    }

    public function deleteConfirmed(): void
    {
        $this->isLoadingDelete = true;
        Designation::findOrFail($this->pendingDeleteId)->delete();
        $this->resetForm();
    ...
    public function createNewDesignation(): void
    {
        $this->redirectRoute('designation.create');
    }
    ...
    <div class="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-full shadow-lg p-4 mb-8 z-10 relative border border-blue-100 dark:border-zinc-800 ring-1 ring-blue-200/30 dark:ring-zinc-700/40">
        <nav class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                <a href="{{ route('designation.manage') }}" class="border rounded-full py-2 px-4 hover:bg-zinc-100 dark:hover:bg-zinc-800 {{ request()->routeIs('designation.manage') ? 'bg-green-600 dark:bg-green-700 text-white dark:text-zinc-200 border-green-400 dark:border-green-500' : '' }}" wire:navigate>
                    {{ __('Designation List') }}
                </a>
                <a href="{{ route('designation.create') }}" class="border rounded-full py-2 px-4 hover:bg-zinc-100 dark:hover:bg-zinc-800 {{ request()->routeIs('designation.create') ? 'bg-green-600 dark:bg-green-700 text-white dark:text-zinc-200 border-green-400 dark:border-green-500' : '' }}" wire:navigate>
                    {{ __('Add Designation') }}
                </a>
            </div>
        </nav>
    ...
    @can('manage_designation')
        ...
        <div class="relative bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-xl shadow-2xl p-6 transition-all duration-300 hover:shadow-3xl border border-blue-100 dark:border-zinc-800 ring-1 ring-blue-200/30 dark:ring-zinc-700/40">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @can('create_designation')
                        <button type="button" wire:click="createNewDesignation"
                            ...
                            <span class="hidden lg:inline">{{ __('Add Designation') }}</span>
                        </button>
                    @endcan
                </div>
            </div>
            ...
            @if (count($selected) > 0)
                <div class="flex items-center justify-between flex-wrap mt-6 p-4 bg-gradient-to-r from-blue-50/80 to-indigo-50/80 dark:from-zinc-800/50 dark:to-zinc-700/50 rounded-xl border border-blue-200 dark:border-zinc-700 backdrop-blur-sm">
                    <div class="flex items-center gap-2 py-2">
                        ...
                        @if(count($selected) < ($this->designations ? $this->designations->total() : 0))
                            <button type="button" wire:click="selectAllData"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline">
                                {{ __('Select all') }} {{ $this->designations ? $this->designations->total() : 0 }} {{ __('items') }}
                            </button>
                        @endif
                    </div>
                    <div class="flex items-center gap-3">
            ...
            <div class="overflow-x-auto bg-transparent mt-6">
                        ...
                        @if($this->shouldShowSkeleton())
                            @for($i = 0; $i < $perPage; $i++)
                                <tr class="animate-pulse border-b border-gray-200 dark:border-gray-700">
                                    ...
                                    <td class="px-5 py-4"><div class="h-4 w-48 bg-blue-100 dark:bg-zinc-800 rounded"></div></td>
                        ...
                        @else
                            @forelse (($this->designations ?? []) as $designation)
                                <tr class="hover:bg-gray-100 dark:hover:bg-white/20 transition group border-b border-gray-200 dark:border-gray-700">
                                    <td class="px-5 py-4">
                                        <input type="checkbox" wire:model.live="selected" value="{{ $designation->id }}" class="accent-pink-500 rounded focus:ring-2 focus:ring-pink-400" />
                                    </td>
                                    <td class="px-5 py-4 text-gray-900 dark:text-white font-bold">{{ $designation->name }}</td>
                                    <td class="px-5 py-4 font-semibold">{{ $designation->code }}</td>
                                    ...
                                    <td class="px-5 py-4">
                                        <span class="flex gap-2">
                                            @can('edit_designation')
                                                <flux:button
                                                    wire:click="confirmEdit({{ $designation->id }})"
                                                    variant="primary"
                                                    color="blue"
                                                    size="sm"
                                                    icon="pencil-square"
                ...
                <div class="mt-6">
                    @if($this->designations && !$this->shouldShowSkeleton())
            ...
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition">
                <div class="bg-white dark:bg-zinc-900 backdrop-blur-xl rounded-2xl shadow-2xl p-8 max-w-md w-full border border-gray-100 dark:border-zinc-800">
                    ...
                    <p class="mb-6 text-zinc-700 dark:text-zinc-300">
                        {{ __('Are you sure you want to delete this designation? This action cannot be undone.') }}
                    </p>
        ...
        @if ($showEditModal)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition">
                <div class="bg-white dark:bg-zinc-900 backdrop-blur-xl rounded-2xl shadow-2xl p-8 max-w-md w-full border border-gray-100 dark:border-zinc-800">
                    ...
                    <p class="mb-6 text-zinc-700 dark:text-zinc-300">
                        {{ __('Are you sure you want to edit this designation?') }}</p>
                    <div class="flex justify-end gap-3">
            ...
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition">
                <div class="bg-gradient-to-br from-pink-50/80 via-white/80 to-red-100/80 dark:from-zinc-900/80 dark:via-zinc-800/80 dark:to-zinc-900/80 backdrop-blur-xl rounded-xl shadow-2xl p-10 max-w-md w-full border border-pink-200 dark:border-zinc-800">
                    ...
                    <p class="mb-6 text-zinc-700 dark:text-zinc-300 font-semibold">
                        {{ __('Are you sure you want to delete the selected designations? This action cannot be undone.') }}
                    </p>
                    <div class="flex justify-end gap-3">
