...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public function updateSelectAllState(): void
    {
        ...
        if ($locations && $locations->count() > 0) {
            $currentPageIds = $locations->pluck('id')->toArray();
    ...
    public function editConfirmed(): void
    {
        ...
        $location = Location::findOrFail($this->pendingEditId);
        $this->showEditModal = false;
        $this->isLoadingEdit = false;
        $this->redirectRoute('location.edit', ['id' => $location->id]);
    }

    public function deleteConfirmed(): void
    {
        $this->isLoadingDelete = true;
        Location::findOrFail($this->pendingDeleteId)->delete();
        $this->resetForm();
        $this->showDeleteModal = false;
        $this->isLoadingDelete = false;

        $this->dispatch('notify', ['type' => 'success', 'message' => __('Location deleted successfully.')]);
    }
    ...
    {
        $this->redirectRoute('location.create');
    }
    ...
    <div class="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-full shadow-lg p-4 mb-8 z-10 relative border border-blue-100 dark:border-zinc-800 ring-1 ring-blue-200/30 dark:ring-zinc-700/40">
        <nav class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                <a href="{{ route('location.manage') }}" class="border rounded-full py-2 px-4 hover:bg-zinc-100 dark:hover:bg-zinc-800 {{ request()->routeIs('location.manage') ? 'bg-green-600 dark:bg-green-700 text-white dark:text-zinc-200 border-green-400 dark:border-green-500' : '' }}" wire:navigate>
                    {{ __('Location List') }}
                </a>
                <a href="{{ route('location.create') }}" class="border rounded-full py-2 px-4 hover:bg-zinc-100 dark:hover:bg-zinc-800 {{ request()->routeIs('location.create') ? 'bg-green-600 dark:bg-green-700 text-white dark:text-zinc-200 border-green-400 dark:border-green-500' : '' }}" wire:navigate>
                    {{ __('Add Location') }}
                </a>
            </div>
        </nav>
    ...
    @can('manage_location')
        ...
        <div class="relative bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-xl shadow-2xl p-6 transition-all duration-300 hover:shadow-3xl border border-blue-100 dark:border-zinc-800 ring-1 ring-blue-200/30 dark:ring-zinc-700/40">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @can('create_location')
                        <button type="button" wire:click="createNewLocation"
                            ...
                            <span class="hidden lg:inline">{{ __('Add Location') }}</span>
                        </button>
                    @endcan
                </div>
            </div>
            ...
            @if (count($selected) > 0)
                <div class="flex items-center justify-between flex-wrap mt-6 p-4 bg-gradient-to-r from-blue-50/80 to-indigo-50/80 dark:from-zinc-800/50 dark:to-zinc-700/50 rounded-xl border border-blue-200 dark:border-zinc-700 backdrop-blur-sm">
                    <div class="flex items-center gap-2 py-2">
                        ...
                        @if(count($selected) < ($this->locations ? $this->locations->total() : 0))
                            <button type="button" wire:click="selectAllData"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline">
                                {{ __('Select all') }} {{ $this->locations ? $this->locations->total() : 0 }} {{ __('items') }}
                            </button>
                        @endif
                    </div>
                    <div class="flex items-center gap-3">
            ...
            <div class="overflow-x-auto bg-transparent mt-6">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                    <thead>
                        <tr class="h-16 bg-zinc-800/5 dark:bg-white/10 text-zinc-600 dark:text-white/70">
                            ...
                            <th class="px-5 py-3 text-left font-semibold uppercase tracking-wider">{{ __('Address') }}</th>
                            <th class="px-5 py-3 text-left font-semibold uppercase tracking-wider cursor-pointer select-none" wire:click="sortBy('city')">
                                {{ __('City') }}
                            ...
                            <th class="px-5 py-3 text-left font-semibold uppercase tracking-wider cursor-pointer select-none" wire:click="sortBy('country')">
                                {{ __('Country') }}
                        ...
                        @if($this->shouldShowSkeleton())
                            @for($i = 0; $i < $perPage; $i++)
                                <tr class="animate-pulse border-b border-gray-200 dark:border-gray-700">
                                    ...
                                    <td class="px-5 py-4"><div class="h-4 w-16 bg-blue-100 dark:bg-zinc-800 rounded"></div></td>
                                    ...
                                    <td class="px-5 py-4"><div class="h-4 w-40 bg-blue-100 dark:bg-zinc-800 rounded"></div></td>
                                    <td class="px-5 py-4"><div class="h-4 w-20 bg-blue-100 dark:bg-zinc-800 rounded"></div></td>
                            ...
                            @forelse (($this->locations ?? []) as $location)
                                <tr class="hover:bg-gray-100 dark:hover:bg-white/20 transition group border-b border-gray-200 dark:border-gray-700">
                                    ...
                                    <td class="px-5 py-4 text-gray-900 dark:text-white font-bold max-w-xs truncate">
                                        <span class="font-semibold text-base">{{ $location->name }}</span>
                                    </td>
                                    ...
                                    <td class="px-5 py-4 max-w-xs truncate">{{ $location->address }}</td>
                                    <td class="px-5 py-4">{{ $location->city }}</td>
                                    <td class="px-5 py-4">{{ $location->country }}</td>
                                    <td class="px-5 py-4 font-semibold">
                                        <span class="truncate text-blue-600 dark:text-blue-400">
                                            {{ $location->created_at ? \Carbon\Carbon::parse($location->created_at)->translatedFormat('j M Y') : '' }}
                                        </span>
                                    </td>
                                    <td class="px-5 py-4">
                                        <span class="flex gap-2">
                                            @can('edit_location')
                                                <flux:button
                                                    wire:click="confirmEdit({{ $location->id }})"
                                                    variant="primary"
                                                    color="blue"
                                                    size="sm"
                                                    icon="pencil-square"
                ...
                <div class="mt-6">
                    @if($this->locations && !$this->shouldShowSkeleton())
            ...
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition">
                <div class="bg-white dark:bg-zinc-900 backdrop-blur-xl rounded-2xl shadow-2xl p-8 max-w-md w-full border border-gray-100 dark:border-zinc-800">
                    ...
                    <p class="mb-6 text-zinc-700 dark:text-zinc-300">
                        {{ __('Are you sure you want to delete this location? This action cannot be undone.') }}
                    </p>
        ...
        @if ($showEditModal)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition">
                <div class="bg-white dark:bg-zinc-900 backdrop-blur-xl rounded-2xl shadow-2xl p-8 max-w-md w-full border border-gray-100 dark:border-zinc-800">
                    ...
                    <p class="mb-6 text-zinc-700 dark:text-zinc-300">
                        {{ __('Are you sure you want to edit this location?') }}</p>
                    <div class="flex justify-end gap-3">
            ...
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition">
                <div class="bg-gradient-to-br from-pink-50/80 via-white/80 to-red-100/80 dark:from-zinc-900/80 dark:via-zinc-800/80 dark:to-zinc-900/80 backdrop-blur-xl rounded-xl shadow-2xl p-10 max-w-md w-full border border-pink-200 dark:border-zinc-800">
                    ...
                    <p class="mb-6 text-zinc-700 dark:text-zinc-300 font-semibold">
                        {{ __('Are you sure you want to delete the selected locations? This action cannot be undone.') }}
                    </p>
                    <div class="flex justify-end gap-3">
