...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public bool $isLoadingImport = false;
    ...
    public $search = '';
    public $filterPermissions = ''; // New filter for permissions
    public $selected = [];
    ...
    public function getRolesProperty()
    {
        ...
        if ($this->filterPermissions) {
            $query->whereHas('permissions', function ($q) {
                $q->where('name', 'like', '%' . $this->filterPermissions . '%');
            });
        }

        $direction = $this->sortDirection === 'asc' ? 'asc' : 'desc';
        $query->orderBy($this->sortField, $direction);
        return $query->with('permissions')->paginate($this->perPage);
    }
    ...
    public function updatedFilterPermissions(): void
    {
    ...
    public function updateSelectAllState(): void
    {
        ...
        if ($roles && $roles->count() > 0) {
            $currentPageIds = $roles->pluck('id')->toArray();
    ...
    public function exportCsv($scope = 'all')
    {
        ...
        $filename = $scope === 'selected' ? 'selected_roles_' . now()->format('Y-m-d_H-i-s') . '.csv' : 'all_roles_' . now()->format('Y-m-d_H-i-s') . '.csv';

        $response = app(\App\Services\ExportService::class)->streamCsv($headers, $items, function ($role) {
            return [
                $role->id,
                $role->name,
                $role->guard_name,
    ...
    public function exportPdf($scope = 'all')
    {
        ...
        $filename = $scope === 'selected' ? 'selected_roles_' . now()->format('Y-m-d_H-i-s') . '.pdf' : 'all_roles_' . now()->format('Y-m-d_H-i-s') . '.pdf';
    ...
    protected function logExport($scope, int $count): void
    {
        Audit::create([
            ...
            'target_type' => 'Spatie\\Permission\\Models\\Role',
            'details' => json_encode($scope === 'selected' ? ['role_ids' => $this->selected] : ['total_roles' => $count]),
        ]);
    }
    ...
    <div class="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-full shadow-lg p-4 mb-8 z-10 relative border border-blue-100 dark:border-zinc-800 ring-1 ring-blue-200/30 dark:ring-zinc-700/40">
        <nav class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                <a href="{{ route('role.index') }}" class="border rounded-full py-2 px-4 hover:bg-zinc-100 dark:hover:bg-zinc-800 {{ request()->routeIs('role.index') ? 'bg-green-600 dark:bg-green-700 text-white dark:text-zinc-200 border-none' : '' }}" wire:navigate>
                    {{ __('Role List') }}
                </a>
                <a href="{{ route('role.show') }}" class="border rounded-full py-2 px-4 hover:bg-zinc-100 dark:hover:bg-zinc-800 {{ request()->routeIs('role.show') ? 'bg-green-600 dark:bg-green-700 text-white dark:text-zinc-200 border-none' : '' }}" wire:navigate>
    ...
    @can('manage_role')
        <div class="relative z-10 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-xl shadow-2xl p-6 transition-all duration-300 hover:shadow-3xl border border-blue-100 dark:border-zinc-800 ring-1 ring-blue-200/30 dark:ring-zinc-700/40">
            <div class="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('role.show') }}" class="flex items-center gap-2 px-2 lg:px-4 py-2 rounded-full border border-blue-200 dark:border-blue-700 text-blue-600 dark:text-blue-400 bg-blue-50/80 dark:bg-blue-900/20 hover:bg-blue-100/80 dark:hover:bg-blue-900/40 shadow-sm backdrop-blur-md focus:outline-none focus:ring-2 focus:ring-blue-400 transition" wire:navigate>
                        ...
                        <span class="hidden lg:inline">{{ __('Add Role') }}</span>
                    </a>
                </div>
            </div>
            ...
            <div>
                @if ($showFilters ?? false)
                    <div class="flex flex-wrap gap-6 mt-6 items-center animate-fade-in">
                        <input type="text" wire:model.live.debounce.300ms="filterPermissions"
                            ...
                            placeholder="{{ __('Filter by permissions...') }}">
                ...
                @if (count($selected) > 0)
                    <div class="flex items-center justify-between flex-wrap mt-6 p-4 bg-gradient-to-r from-blue-50/80 to-indigo-50/80 dark:from-zinc-800/50 dark:to-zinc-700/50 rounded-xl border border-blue-200 dark:border-zinc-700 backdrop-blur-sm">
                        <div class="flex items-center gap-2 py-2">
                            ...
                            @if(count($selected) < ($this->roles ? $this->roles->total() : 0))
                                <button type="button" wire:click="selectAllData"
                                    class="text-sm text-blue-600 dark:text-blue-400 hover:underline">
                                    {{ __('Select all') }} {{ $this->roles ? $this->roles->total() : 0 }} {{ __('items') }}
                                </button>
                            @endif
                        </div>
                        <div class="flex items-center gap-3">
            ...
            <div class="overflow-x-auto bg-transparent mt-6">
                            ...
                            @forelse (($this->roles ?? []) as $role)
                                <tr class="hover:bg-gray-100 dark:hover:bg-white/20 transition group border-b border-gray-200 dark:border-gray-700">
                                    ...
                                    <td class="px-5 py-4 text-gray-900 dark:text-white font-bold">
                                        <span class="group-hover:underline">{{ $role->name }}</span>
                                    </td>
                                    <td class="px-5 py-4">
                                        <div class="flex flex-wrap gap-2">
                                            @forelse($role->permissions as $permission)
                                                <span class="inline-block bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 px-2 py-1 rounded text-xs font-medium">{{ $permission->name }}</span>
                                            @empty
                                                <span class="text-gray-500 dark:text-gray-400 text-sm italic">{{ __('No permissions') }}</span>
                                            @endforelse
                                        </div>
                                    </td>
                ...
                <div class="mt-6">
                    @if($this->roles && !$this->shouldShowSkeleton())
                        {{ $this->roles->links() }}
                    @endif
                </div>
            </div>
        ...
        @if ($showEditModal)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition">
                <div class="bg-white dark:bg-zinc-900 backdrop-blur-xl rounded-2xl shadow-2xl p-8 max-w-md w-full border border-gray-100 dark:border-zinc-800">
                    ...
                    <p class="mb-6 text-zinc-700 dark:text-zinc-300">
                        {{ __('Are you sure you want to edit this role?') }}
                    </p>
                    <div class="flex justify-end gap-3">
            ...
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition">
                <div class="bg-gradient-to-br from-pink-50/80 via-white/80 to-red-100/80 dark:from-zinc-900/80 dark:via-zinc-800/80 dark:to-zinc-900/80 backdrop-blur-xl rounded-xl shadow-2xl p-10 max-w-md w-full border border-pink-200 dark:border-zinc-800">
                    ...
                    <p class="mb-6 text-zinc-700 dark:text-zinc-300 font-semibold">
                        {{ __('Are you sure you want to delete the selected roles? This action cannot be undone.') }}
                    </p>
                    <div class="flex justify-end gap-3">
